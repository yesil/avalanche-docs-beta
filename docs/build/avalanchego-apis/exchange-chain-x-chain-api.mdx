...
Mint units of a variable-cap asset created with [`avm.createVariableCapAsset`](exchange-chain-x-chain-api.mdx#avm-createvariablecapasset).
...
Mint non-fungible tokens which were created with [`avm.createNFTAsset`](exchange-chain-x-chain-api.mdx#avm-createnftasset). More details can be found [here](../tutorials/smart-digital-assets/creating-a-nft-part-1.md).
...
The returned private key can be added to a user with [`avm.importKey`](exchange-chain-x-chain-api.mdx#avm-importkey).
...
### avm.getAddressTxs {#avm-get-address-txs-api}
